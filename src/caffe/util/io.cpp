  ...
  BOOST_FOREACH(ptree::value_type &v1, pt.get_child("annotation")) {
    ...
    if (v1.first == "object") {
      ...
      BOOST_FOREACH(ptree::value_type &v2, object.get_child("")) {
        ...
        if (v2.first == "name") {
        ...
        } else if (v2.first == "content") {
          text_content = pt2.data();
          ...
          NormalizedPolygon* polygon = anno->mutable_polygon();
          ...
          // rbox->set_x1(static_cast<float>(x1) / width);
          // rbox->set_y1(static_cast<float>(y1) / height);
          // rbox->set_x2(static_cast<float>(x2) / width);
          // rbox->set_y2(static_cast<float>(y2) / height);
          // rbox->set_h(static_cast<float>(h) / height);
          ...
          polygon->set_x1(static_cast<float>(x1) / width);
          polygon->set_y1(static_cast<float>(y1) / height);
          polygon->set_x2(static_cast<float>(x2) / width);
          polygon->set_y2(static_cast<float>(y2) / height);
          polygon->set_x3(static_cast<float>(x3) / width);
          polygon->set_y3(static_cast<float>(y3) / height);
          polygon->set_x4(static_cast<float>(x4) / width);
          polygon->set_y4(static_cast<float>(y4) / height);
          // NormalizedBBox* bbox = anno->mutable_bbox();
          // NormalizedRBox* rbox = anno->mutable_rbox();
          // bbox->set_xmin(static_cast<float>(xmin) / width);
          // bbox->set_ymin(static_cast<float>(ymin) / height);
          ...
          // float x3 = x2 + std::cos(theta + M_PI / 2.) * h;
          // float y3 = y2 + std::sin(theta + M_PI / 2.) * h;
          // float new_x1 = static_cast<float>(x1) / width;
          // float new_y1 = static_cast<float>(y1) / height;
          // float new_x2 = static_cast<float>(x2) / width;
          // float new_y2 = static_cast<float>(y2) / height;
          // float new_x3 = static_cast<float>(x3) / width;
          // float new_y3 = static_cast<float>(y3) / height;
          // float new_h = std::sqrt((new_x3 - new_x2) * (new_x3 - new_x2) + (new_y3 - new_y2) * (new_y3 - new_y2));
          // rbox->set_x1(new_x1);
          // rbox->set_y1(new_y1);
          // rbox->set_x2(new_x2);
          // rbox->set_y2(new_y2);
...
int ascii2label(int ascii){
  int label = 0;
  if(ascii >= 48 && ascii <= 57){
    // '0'~'9' are mapped to 1~10
    label = ascii - 47;
  }else if(ascii >= 65 && ascii <= 90){
    // 'A'-'Z' are mapped to 11-36
    label = ascii - 64 + 10;
...
void string2labels(const string s, int* labels, const int maxLength){
  for(unsigned i=0; i<s.size(); i++){
    char c = s[i];
    labels[i]=ascii2label(int(c));
  }
  for(unsigned j=s.size(); j<maxLength; j++){
    labels[j]=0;
  }
}

int label2ascii(int label){
  int ascii = 0;
  if(label >=1 && label <= 10){
    ascii = label - 1 + 48;
  }else if(label >= 11 && label <= 36){
    ascii = label - 11 + 65;
  }else if(label == 0){
    ascii = 45;
  }
...
void labels2string(const int* labels, string& s){
  for(int i = 0; i < sizeof(labels)/sizeof(int); i++){
    if(labels[i] == 0){
      break;
    }else{
      s[i]=label2ascii(labels[i]);
    }
  }
}
