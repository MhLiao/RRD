...
def BlstmLayer(net, from_layer, num_hidden, num_output, prefix):
    # input: 26*512
    # lstm1: 512->256->256
    # lstm2: 256->256->37(26+10+1)
  lstm_name1=prefix+'_'+'lstm1'
  net[lstm_name1] = L.LSTM(net[from_layer], num_output=num_hidden,  
    bias_filler=dict(type='constant', value=0), weight_filler=dict(type='gaussian', std=0.1))
  ip_name1=prefix+'_'+'ip1'
  net[ip_name1] = L.InnerProduct(net[lstm_name1], num_output=num_output, weight_filler=dict(type='gaussian', std=0.1))
  reverse_name1=prefix+'_'+'rev1'
  net[reverse_name1] = L.Reverse(net[from_layer])
  lstm_name2=prefix+'_'+'lstm2'
  net[lstm_name2] = L.LSTM(net[reverse_name1], num_output=num_hidden, 
    bias_filler=dict(type='constant', value=0), weight_filler=dict(type='gaussian', std=0.1))
  ip_name2=prefix+'_'+'ip2'
  net[ip_name2] = L.InnerProduct(net[lstm_name2], num_output=num_output, weight_filler=dict(type='gaussian', std=0.1))
  reverse_name2=prefix+'_'+'rev2'
  net[reverse_name2] = L.Reverse(net[ip_name2])
  Eltwise_name=prefix+'_'+'eltwise'
  L.Eltwise(net[ip_name1], net[reverse_name2], operation=P.Eltwise.SUM)


def ConvBNLayer(net, from_layer, out_layer, use_bn, use_relu, num_output,
    kernel_size, pad, stride, dilation=1, use_scale=True, lr_mult=1, orientations = 0, 
  ...
  if use_bn:
    if orientations > 0:
      ...
      kwargs = {
          'orn_param': dict(orientations=orientations),
...
def VGGNetBody(net, from_layer, need_fc=True, fully_conv=False, reduced=False, orientations=0,
    ...
    if orientations > 0:
      extend_kwargs = {
              'orn_param': dict(orientations=orientations)}
      net.extend = L.Extend(net[from_layer], **extend_kwargs)
      net.conv1_1 = L.Convolution(net.extend, num_output=64, pad=1, kernel_size=3, **kwargs)
    else:
...
def VGGNetBody_orientation(net, from_layer, need_fc=True, fully_conv=False, reduced=False, orientations=0,
        dilated=False, nopool=False, dropout=True, freeze_layers=[], dilate_pool4=False, wider_factor = 2):
    if orientations > 0:
      kwargs = {
              'orn_param': dict(orientations=orientations),
              'param': [dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)],
    ...
    if orientations > 0:
      extend_kwargs = {
              'orn_param': dict(orientations=orientations)}
      net.extend = L.Extend(net[from_layer], **extend_kwargs)
      net.conv1_1 = L.Convolution(net.extend, num_output=wider_factor*64, pad=1, kernel_size=3, **kwargs)
    else:
      net.conv1_1 = L.Convolution(net[from_layer], num_output=wider_factor*64, pad=1, kernel_size=3, **kwargs)

    net.relu1_1 = L.ReLU(net.conv1_1, in_place=True)
    net.conv1_2 = L.Convolution(net.relu1_1, num_output=wider_factor*64, pad=1, kernel_size=3, **kwargs)
    net.relu1_2 = L.ReLU(net.conv1_2, in_place=True)

    if nopool:
        name = 'conv1_3'
        net[name] = L.Convolution(net.relu1_2, num_output=wider_factor*64, pad=1, kernel_size=3, stride=2, **kwargs)
    else:
        name = 'pool1'
        net.pool1 = L.Pooling(net.relu1_2, pool=P.Pooling.MAX, kernel_size=2, stride=2)

    net.conv2_1 = L.Convolution(net[name], num_output=wider_factor*128, pad=1, kernel_size=3, **kwargs)
    net.relu2_1 = L.ReLU(net.conv2_1, in_place=True)
    net.conv2_2 = L.Convolution(net.relu2_1, num_output=wider_factor*128, pad=1, kernel_size=3, **kwargs)
    net.relu2_2 = L.ReLU(net.conv2_2, in_place=True)

    if nopool:
        name = 'conv2_3'
        net[name] = L.Convolution(net.relu2_2, num_output=wider_factor*128, pad=1, kernel_size=3, stride=2, **kwargs)
    else:
        name = 'pool2'
        net[name] = L.Pooling(net.relu2_2, pool=P.Pooling.MAX, kernel_size=2, stride=2)

    net.conv3_1 = L.Convolution(net[name], num_output=wider_factor*256, pad=1, kernel_size=3, **kwargs)
    net.relu3_1 = L.ReLU(net.conv3_1, in_place=True)
    net.conv3_2 = L.Convolution(net.relu3_1, num_output=wider_factor*256, pad=1, kernel_size=3, **kwargs)
    net.relu3_2 = L.ReLU(net.conv3_2, in_place=True)
    net.conv3_3 = L.Convolution(net.relu3_2, num_output=wider_factor*256, pad=1, kernel_size=3, **kwargs)
    net.relu3_3 = L.ReLU(net.conv3_3, in_place=True)

    if nopool:
        name = 'conv3_4'
        net[name] = L.Convolution(net.relu3_3, num_output=wider_factor*256, pad=1, kernel_size=3, stride=2, **kwargs)
    else:
        name = 'pool3'
        net[name] = L.Pooling(net.relu3_3, pool=P.Pooling.MAX, kernel_size=2, stride=2)

    net.conv4_1 = L.Convolution(net[name], num_output=wider_factor*512, pad=1, kernel_size=3, **kwargs)
    net.relu4_1 = L.ReLU(net.conv4_1, in_place=True)
    net.conv4_2 = L.Convolution(net.relu4_1, num_output=wider_factor*512, pad=1, kernel_size=3, **kwargs)
    net.relu4_2 = L.ReLU(net.conv4_2, in_place=True)
    net.conv4_3 = L.Convolution(net.relu4_2, num_output=wider_factor*512, pad=1, kernel_size=3, **kwargs)
    net.relu4_3 = L.ReLU(net.conv4_3, in_place=True)

    if nopool:
        name = 'conv4_4'
        net[name] = L.Convolution(net.relu4_3, num_output=wider_factor*512, pad=1, kernel_size=3, stride=2, **kwargs)
    else:
        name = 'pool4'
        if dilate_pool4:
            net[name] = L.Pooling(net.relu4_3, pool=P.Pooling.MAX, kernel_size=3, stride=1, pad=1)
    ...
    kernel_size = 3
    pad = int((kernel_size + (dilation - 1) * (kernel_size - 1)) - 1) / 2
    net.conv5_1 = L.Convolution(net[name], num_output=wider_factor*512, pad=pad, kernel_size=kernel_size, dilation=dilation, **kwargs)
    net.relu5_1 = L.ReLU(net.conv5_1, in_place=True)
    net.conv5_2 = L.Convolution(net.relu5_1, num_output=wider_factor*512, pad=pad, kernel_size=kernel_size, dilation=dilation, **kwargs)
    net.relu5_2 = L.ReLU(net.conv5_2, in_place=True)
    net.conv5_3 = L.Convolution(net.relu5_2, num_output=wider_factor*512, pad=pad, kernel_size=kernel_size, dilation=dilation, **kwargs)
    net.relu5_3 = L.ReLU(net.conv5_3, in_place=True)
    ...
    if need_fc:
        if dilated:
            if nopool:
                name = 'conv5_4'
                net[name] = L.Convolution(net.relu5_3, num_output=wider_factor*512, pad=1, kernel_size=3, stride=1, **kwargs)
            else:
                name = 'pool5'
                net[name] = L.Pooling(net.relu5_3, pool=P.Pooling.MAX, pad=1, kernel_size=3, stride=1)
        else:
            if nopool:
                name = 'conv5_4'
                net[name] = L.Convolution(net.relu5_3, num_output=wider_factor*512, pad=1, kernel_size=3, stride=2, **kwargs)
            else:
                name = 'pool5'
                net[name] = L.Pooling(net.relu5_3, pool=P.Pooling.MAX, kernel_size=2, stride=2)
        ...
        if fully_conv:
            if dilated:
                if reduced:
                    dilation = dilation * 6
                    kernel_size = 3
                    num_output = wider_factor * 1024
                else:
                    dilation = dilation * 2
                    kernel_size = 7
                    num_output = wider_factor * 4096
            else:
                if reduced:
                    dilation = dilation * 3
                    kernel_size = 3
            ...
            if reduced:
                net.fc7 = L.Convolution(net.relu6, num_output=wider_factor * 1024, kernel_size=1, **kwargs)
            else:
                net.fc7 = L.Convolution(net.relu6, num_output=wider_factor * 4096, kernel_size=1, **kwargs)
            net.relu7 = L.ReLU(net.fc7, in_place=True)
            if dropout:
                net.drop7 = L.Dropout(net.relu7, dropout_ratio=0.5, in_place=True)
        else:
            net.fc6 = L.InnerProduct(net.pool5, num_output=wider_factor * 4096)
            net.relu6 = L.ReLU(net.fc6, in_place=True)
            if dropout:
                net.drop6 = L.Dropout(net.relu6, dropout_ratio=0.5, in_place=True)
            net.fc7 = L.InnerProduct(net.relu6, num_output=wider_factor * 4096)
            net.relu7 = L.ReLU(net.fc7, in_place=True)
            if dropout:
                net.drop7 = L.Dropout(net.relu7, dropout_ratio=0.5, in_place=True)
...
def CreateMultiBoxHead_multitask(net, data_layer="data", num_classes=[], from_layers=[],
    ...
    for i in range(0, num):
        ...
        name = "{}_mbox_priorbox".format(from_layer)
        net[name] = L.PriorBox(net[from_layer], net[data_layer], min_size=min_size,
                clip=clip, variance=prior_variance, offset=offset, denser_prior_boxes=denser_prior_boxes)
        if max_size:
            net.update(name, {'max_size': max_size})
        if aspect_ratio:
            net.update(name, {'aspect_ratio': aspect_ratio, 'flip': flip})
...
def CreateMultiBoxHead_orn(net, data_layer="data", num_classes=[], from_layers=[],
        use_objectness=False, normalizations=[], use_batchnorm=True, lr_mult=1, use_polygon = True,
        use_scale=True, min_sizes=[], max_sizes=[], prior_variance = [0.1], denser_prior_boxes=False,
        aspect_ratios=[], steps=[], img_height=0, img_width=0, share_location=True,
        flip=True, clip=True, offset=0.5, inter_layer_depth=[], kernel_size=1, pad=0, orientations = 0,
        conf_postfix='', loc_postfix='', **bn_param):
    assert num_classes, "must provide num_classes"
    assert num_classes > 0, "num_classes must be positive number"
    if normalizations:
    ...
    for i in range(0, num):
        ...
        # Create confidence prediction layer.
        orpooling_name = "{}_mbox_conf_orpooling{}".format(from_layer, conf_postfix)
        if orientations > 0:
          orpooling_kwargs = {'orn_param': dict(orientations=orientations),}
        net[orpooling_name] = L.ORPooling(net[from_layer], **orpooling_kwargs)
        name = "{}_mbox_conf{}".format(from_layer, conf_postfix)
        num_conf_output = num_priors_per_location * num_classes;
        ConvBNLayer(net, orpooling_name, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
            num_output=num_conf_output, kernel_size=kernel_size, pad=pad, stride=1, **bn_param)
        permute_name = "{}_perm".format(name)
        net[permute_name] = L.Permute(net[name], order=[0, 2, 3, 1])
        flatten_name = "{}_flat".format(name)
...
def CreateMultiBoxHead_orn_inception(net, data_layer="data", num_classes=[], cls_from_layers=[], reg_from_layers=[],
        use_objectness=False, normalizations=[], use_batchnorm=True, lr_mult=1, use_polygon = True,
        use_scale=True, min_sizes=[], max_sizes=[], prior_variance = [0.1], denser_prior_boxes=False,
        aspect_ratios=[], steps=[], img_height=0, img_width=0, share_location=True,
        flip=True, clip=True, offset=0.5, inter_layer_depth=[], kernel_size=1, pad=0, orientations = 0,
    ...
    assert num_classes > 0, "num_classes must be positive number"
    if normalizations:
        assert len(cls_from_layers) == len(normalizations), "cls_from_layers and normalizations should have same length"
    assert len(cls_from_layers) == len(min_sizes), "cls_from_layers and min_sizes should have same length"
    if max_sizes:
        assert len(cls_from_layers) == len(max_sizes), "cls_from_layers and max_sizes should have same length"
    if aspect_ratios:
        assert len(cls_from_layers) == len(aspect_ratios), "cls_from_layers and aspect_ratios should have same length"
    if steps:
        assert len(cls_from_layers) == len(steps), "cls_from_layers and steps should have same length"
    net_layers = net.keys()
    assert data_layer in net_layers, "data_layer is not in net's layers"
    if inter_layer_depth:
        assert len(cls_from_layers) == len(inter_layer_depth), "cls_from_layers and inter_layer_depth should have same length"

    num = len(cls_from_layers)
    priorbox_layers = []
    loc_layers = []
    conf_layers = []
    objectness_layers = []
    for i in range(0, num):
        from_layer = cls_from_layers[i]
